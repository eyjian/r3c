...
{
    const std::string lua_scripts = "local n;n=redis.call('HSETNX',KEYS[1],ARGV[1],ARGV[2]);if (n>0) then redis.call('EXPIRE',KEYS[1],ARGV[3]) end;return n;";
    std::vector<std::string> parameters(3);
    parameters[0] = field;
    parameters[1] = value;
    parameters[2] = any2string(expired_seconds);
...
const RedisReplyHelper CRedisClient::redis_command(bool is_read_command, bool force_retry, int retry_times, const std::string& key, const CCommandArgs& command_args, std::pair<std::string, uint16_t>* which)
{
    ...
    if (key.empty() && cluster_mode())
    {
        ...
        errinfo.raw_errmsg = format_string("[SLOT:%d][%s] key not set in cluster mode", slot, command_args.get_command());
